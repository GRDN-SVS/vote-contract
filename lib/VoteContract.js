...
class VoteContract extends Contract {
    ...
    async createVoter(ctx, args) {
        ...
        let response = {};
        response.success = `voter with voterId ${newVoter.voterId} has been successfully registered`;
        return response;
    }
    ...
    async castVote(ctx, args) {
        ...
        if (electionExists) {
            ...
            if (parsedCurrentTime >= electionStart && parsedCurrentTime < electionEnd) {
                ...
                await ctx.stub.putState(voter.voterId, Buffer.from(JSON.stringify(voter)));
                let response = {};
                response.success = `voter with voterId ${voter.voterId} has successfully voted`;
                return response;
            }
            else {
                let response = {};
